...
app.post("/api/send-email", async (req, res) => {
  ...
  if (!to_email || !message) {
    return res
      .status(400)
      .json({ error: "Faltan datos para enviar el correo." });
  }
  ...
  if (
    ...
    console.error(
      "❌ Variables de EmailJS no están completamente definidas en el entorno."
    );
    return res
      .status(500)
      .json({
        error:
          "Error de configuración del servidor: Faltan credenciales de EmailJS.",
      });
  }
...
app.listen(PORT, () => {
  ...
  console.log(
    `🌐 Accede al frontend en: http://localhost:${PORT} (si el backend sirve el frontend)`
  );
  console.log(
    `📡 Endpoint de configuración API: http://localhost:${PORT}/api/config`
  );
});
